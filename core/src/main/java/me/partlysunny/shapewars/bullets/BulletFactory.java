...
public final class BulletFactory {
    ...
    public Entity generateBullet(PooledEngine world, BulletType type, float damage, Entity shooter, BulletController controller) {
        ...
        float x = MathUtils.cos(rotation) * 4;
        float y = MathUtils.sin(rotation) * 4;
        rigidBodyComponent.initBody(shooterPos.position.x + x, shooterPos.position.y + y, shooterPos.rotation, type.hitbox().genDef(), BodyDef.BodyType.DynamicBody);
